...
if ($action == "get_offices") {
    ...
    // ayaw pahalata ba nga wa sab kabalo kay utro sab ko aha nimo ni gikuha
    // $sql="SELECT *,offices.ID AS 'office_id' FROM offices INNER JOIN queue ON queue.office = offices.ID WHERE offices.availability = 'available' AND queue.date_of_transaction = '".date('Y-m-d')."' GROUP BY offices.name  ORDER BY offices.ID DESC";
