    ...
    $(document).ready(()=>{
        $.post("ajax/get_numbers.php",{
            ...
            offices.forEach(element => {
                ...
                center_content.append($("<div class='office_row'></div>").append($("<div class='office_name'>"+element.officeName+"</div>")
                ).append($("<h5  >Unavailable</h5>")));
                }
                else{
                center_content.append($("<div class='office_row'></div>").append($("<div class='office_name'>"+element.officeName+"</div>")
                ).append($("<button id = 'print_button' class='print_button' >Print</button>").click((event)=>{
                    //function for printing number
                    $.post("admin/ajax.php",{action:"get_office_availability",officeID:element.officeID},(availability)=>{
                        if (availability == "available") {
                        $.post("ajax/get_numbers.php",{office:element.officeID,action:'get_queue'},
                        ((data,status)=>{
                            ...
                            .append($("<h6>This is only valid until: "+new Date(Date.now()).toDateString()+"</h6>")).printThis();
                                ...
                                $.post("ajax/get_numbers.php",{
                                    ...
                                    $.post("ajax/get_numbers.php",{
                                        office:element.officeID,action:'get_queue'
                                    },((data,status)=>{
                ...
                ,$("<button id = 'priority_queue' class='priority_queue' >Priority</button>").click((event)=>{
                    $.post("admin/ajax.php",{action:"get_office_availability",officeID:element.officeID},(availability)=>{
                        if (availability == "available") {
                    $.post("ajax/get_numbers.php",{office:element.officeID,action:'get_priority_queue'},
                    ((priority_data,status)=>{
                            ...
                            .append($("<h6>This is only valid until: "+new Date(Date.now()).toDateString()+"</h6>")).printThis();
                                ...
                                $.post("ajax/get_numbers.php",{
                                    ...
                                    $.post("ajax/get_numbers.php",{
                                        office:element.officeID,action:'get_priority_queue'
                                    },((priority_data,status)=>{
